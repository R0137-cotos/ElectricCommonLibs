...
|256|契約作成(低圧)時の契約担当・請求担当の氏名・メールアドレスを必須に変更|
|257|計上実績に負荷率を追加|
|258|請求基本情報にRJ部門名、振込先銀行名、振込先銀行支店名を追加|
|259|契約情報作成(低圧)・契約情報プラン変更(低圧)時の基本検針日を必須に変更|
|260|計上実績取得結果Dtoに項目追加|
|261|計上実績取得結果Dtoの論理名・物理名不一致を修正|
|262|請求基本情報に自振口座_口座種別を追加|
|263|契約（電力用）に契約番号フリー入力フラグを追加|
|264|契約（電力用）リスト取得用Dtoに得意先CD、請求No追加|
|265|検索パラメータに「更新案件は除く」を追加|
|266|（洗替用）企業情報を追加|
|267|（洗替用）企業情報Dtoを追加|
|268|MyRICOH連携実績を追加|
|269|請求実績に請求書発行年月日を追加|
|270|非請求理由の文字長を変更|
|271|契約（電力用）に締め日カレンダーチェックフラグを追加|
|272|契約書署名マスタを追加|
|273|契約書最終作成日にカラム名変更|
|274|計上実績に再エネ賦課金減免カラム追加|
|275|精算分電力量差異情報に再エネ賦課金減免カラム追加|
|276|契約（電力用）に契約締結促進メールフラグを追加|
|277|契約（電力用）に契約書未返送アラート判定基準日を追加|
|278|解約情報に訪問予定日・変更先電力会社を追加|
...
|291|解約詳細情報、解約理由情報エンティティを追加|
|292|解約情報に解約金額分類を追加|
|293|契約（電力）添付ファイルにマスタープリセットフラグを追加、需要消滅廃止理由追加|
|294|廃止手配結果登録エラーメッセージを追加|
|295|電気事業者マスタ追加|
|296|請求実績に請求メール送信日時を追加|
|297|請求書通知メールファイルテーブルを追加|
|298|請求書通知メールファイルテーブルのシーケンスを廃止|
|299|請求実績一覧情報取得結果にメール送信日時、ファイルID、ファイル名を追加する|
|300|廃止理由に供給前解約を追加|
