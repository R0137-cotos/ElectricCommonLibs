...
|253|2020/07/27リリース分マージ|
|254|403エラーのメッセージを日本語化|
|255|403エラーのメッセージを戻し|
|256|認可判定ユーザがRJ社員マスタに存在しない場合判定から除外するよう修正|
|257|契約作成(低圧)時の契約担当・請求担当の氏名・メールアドレスを必須に変更|
|258|計上実績に負荷率を追加|
|259|請求基本情報にRJ部門名、振込先銀行名、振込先銀行支店名を追加|
|260|契約情報作成(低圧)・契約情報プラン変更(低圧)時の基本検針日を必須に変更|
|261|計上実績取得結果Dtoに項目追加|
|262|計上実績取得結果Dtoの論理名・物理名不一致を修正|
|263|請求基本情報に自振口座_口座種別を追加|
|264|契約（電力用）に契約番号フリー入力フラグを追加|
|265|契約（電力用）リスト取得用Dtoに得意先CD、請求No追加|
|266|検索パラメータに「更新案件は除く」を追加|
|267|（洗替用）企業情報を追加|
|268|（洗替用）企業情報Dtoを追加|
|269|MyRICOH連携実績を追加|
|270|請求実績に請求書発行年月日を追加|
|271|非請求理由の文字長を変更|
|272|契約（電力用）に締め日カレンダーチェックフラグを追加|
|273|契約書署名マスタを追加|
|274|契約書最終作成日にカラム名変更|
|275|契約（電力用）に契約締結促進メールフラグを追加|
|276|契約（電力用）に契約書未返送アラート判定基準日を追加|
|277|計上実績に再エネ賦課金減免カラム追加|
|278|精算分電力量差異情報に再エネ賦課金減免カラム追加|
|279|請求書発送区分に特殊加工請求を追加|
|280|引落前連絡のコンバーターを実装|
|281|手配結果登録DTOに解約日を追加|
|282|解約情報に手配結果登録日を追加／解約情報に訪問予定日・変更先電力会社を追加|
|283|契約（電力用）に現在の契約番号入力フォーム識別子を追加|
|284|解約情報に（解約手続時点）需給（供給）期間 終了日を追加、手配結果登録パラメータに解約指定時刻を追加|
|285|解約情報の手配結果登録日の論理名を変更、手配結果登録パラメータに連携パターンを追加|
|286|重要事項説明者に媒介用項目追加|
|287|契約（電力用）に契約満了前チェックフラグを追加|
|288|契約満了前チェックエラーメッセージ変更|
|289|契約満了前チェックエラーメッセージ変更（事業区確認後）|
|290|契約満了前チェックエラーメッセージ変更（事業区確認後）|
|291|廃止手配結果登録エラーメッセージを追加|
|292|解約情報に訪問予定日・変更先電力会社を追加|
|293|解約詳細情報、解約理由情報エンティティを追加|
|294|解約情報に解約金額分類を追加|
|295|契約（電力）添付ファイルにマスタープリセットフラグを追加、需要消滅廃止理由追加|
|296|電気事業者マスタ追加|
|297|請求実績に請求メール送信日時を追加|
|298|請求書通知メールファイルテーブルを追加|
|299|請求書通知メールファイルテーブルのシーケンスを廃止|
|300|請求実績一覧情報取得結果にメール送信日時、ファイルID、ファイル名を追加する|
|301|契約（電力用）に必須表示区分を追加|
|302|MoM権限サービスRIZM対応（テストのみ）|
|303|[更新用]案件担当者更新用テーブル追加|
|304|[更新用]単価情報テーブル追加|
|305|[更新用]単価情報テーブルに備考追加|
|306|MFP販促対応マスタ追加|
|307|MFP販促対応マスタ追加(誤記修正)|