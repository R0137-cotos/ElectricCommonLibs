...
|280|引落前連絡のコンバーターを実装|
|281|契約（電力用）に現在の契約番号入力フォーム識別子を追加|
|282|重要事項説明者に媒介用項目追加|
|283|手配結果登録DTOに解約日を追加|
|284|解約情報に手配結果登録日を追加|
|285|解約情報に（解約手続時点）需給（供給）期間 終了日を追加、手配結果登録パラメータに解約指定時刻を追加|
|286|解約情報の手配結果登録日の論理名を変更、手配結果登録パラメータに連携パターンを追加|
...
|292|請求実績に請求メール送信日時を追加|
|293|請求書通知メールファイルテーブルを追加|
|294|請求書通知メールファイルテーブルのシーケンスを廃止|
|295|請求実績一覧情報取得結果にメール送信日時、ファイルID、ファイル名を追加する|
|296|解約情報に訪問予定日・変更先電力会社を追加|
|297|解約詳細情報、解約理由情報エンティティを追加|
|298|解約情報に解約金額分類を追加|
|299|契約（電力）添付ファイルにマスタープリセットフラグを追加、需要消滅廃止理由追加|
|300|電気事業者マスタ追加|
|301|契約（電力用）に必須表示区分を追加|
|302|[更新用]案件担当者更新用テーブル追加|
|303|[更新用]単価情報テーブル追加|
|304|[更新用]単価情報テーブルに備考追加|
|305|MFP販促対応マスタ追加|
|306|EOSL対応|
