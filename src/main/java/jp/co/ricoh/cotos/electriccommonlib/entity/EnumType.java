...
public class EnumType {
	...
	public enum SendInvoiceDiv {
		...
		// メール+MyRICOH
		メール_MyRICOH("1"), メール("2"), 紙請求("3"), メール_紙請求("4"), MyRICOH("5"), 特殊加工請求("6");
	...
	public enum RequiredIndicationDiv {

		帳票出力("1"), 承認依頼("2");

		private final String text;

		private RequiredIndicationDiv(final String text) {
			this.text = text;
		}
		...
		@JsonCreator
		public static RequiredIndicationDiv fromString(String string) {
			return Arrays.stream(values()).filter(v -> v.text.equals(string)).findFirst().orElseThrow(() -> new IllegalArgumentException(String.valueOf(string)));
		}
	}

	public enum TyohyoKbn {

		標準("0"), スターツ("1");

		private final String text;

		private TyohyoKbn(final String text) {
			this.text = text;
		}
		...
		@JsonCreator
		public static TyohyoKbn fromString(String string) {
			return Arrays.stream(values()).filter(v -> v.text.equals(string)).findFirst().orElseThrow(() -> new IllegalArgumentException(String.valueOf(string)));
		}
	}
}
